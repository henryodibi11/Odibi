...
class AzureOpenAIClient:
    ...
    def chat_completion(
        ...
        if response.status_code != 200:
            raise RuntimeError(
                f"Azure OpenAI chat completion failed: " f"{response.status_code} - {response.text}"
            )
    ...
    def create_embeddings(
        ...
        for i in range(0, len(texts), batch_size):
            ...
            if response.status_code != 200:
                raise RuntimeError(
                    f"Azure OpenAI embeddings failed: " f"{response.status_code} - {response.text}"
                )
...
class AzureSearchClient:
    ...
    def upload_documents(
        ...
        for i in range(0, len(documents), batch_size):
            ...
            if response.status_code not in (200, 207):
                raise RuntimeError(
                    f"Azure AI Search upload failed: " f"{response.status_code} - {response.text}"
                )
    ...
    def create_index(self, index_schema: dict[str, Any]) -> bool:
        ...
        if response.status_code not in (200, 201):
            raise RuntimeError(
                f"Azure AI Search index creation failed: "
    ...
    def delete_index(self, index_name: Optional[str] = None) -> bool:
        ...
        if response.status_code not in (204, 404):
            raise RuntimeError(
                f"Azure AI Search index deletion failed: "
                f"{response.status_code} - {response.text}"
            )
