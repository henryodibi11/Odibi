...
4. Cycle-based scheduled assistant mode
"""
...
from odibi.agents.prompts.cycle_agents import register_cycle_agents
from odibi.agents.prompts.orchestrator import create_agent_suite


@dataclass
class ScheduledConfig:
    """Configuration for scheduled assistant mode."""
...
class AgentRunnerConfig:
    ...
    default_mode: str = AssistantMode.INTERACTIVE.value
...
class AgentRunner:
    ...
    Supports three assistant modes:
    1. Interactive - Answer questions, help author YAML (existing behavior)
    2. Guided Execution - Run specific pipelines, validate configs
    3. Scheduled Assistant - Background work with bounded cycles
    """
    ...
    def __init__(
        ...
        self.mode = AssistantMode(self.runner_config.default_mode)
        ...
        register_cycle_agents(azure_config)

        self.cycle_store = CycleStore(self.runner_config.memory_path)
    ...
    def get_mode(self) -> AssistantMode:
        """Get the current assistant mode.
    ...
    def start_guided_cycle(
        self,
        project_root: str,
        task_description: str = "Guided execution cycle",
        max_improvements: int = 3,
        golden_projects: Optional[list] = None,
    ) -> CycleState:
        """Start a guided execution cycle.

        Args:
            project_root: Root directory for the project.
            task_description: Description of the task.
            max_improvements: Maximum improvements to propose.
            golden_projects: Golden projects for regression checks.
                Can be: list of strings, list of GoldenProjectConfig, or
                comma-separated string.

        Returns:
            Initial CycleState.
        """
        parsed_golden = parse_golden_projects_input(golden_projects or [], project_root)

        config = CycleConfig(
            project_root=project_root,
            task_description=task_description,
            max_improvements=max_improvements,
            golden_projects=parsed_golden,
        )
        return self.cycle_runner.start_cycle(config, AssistantMode.GUIDED_EXECUTION)

    def continue_cycle(self, cycle_id: str) -> CycleState:
        """Continue an existing cycle.

        Args:
            cycle_id: The cycle ID to continue.
        ...
        if state.is_finished():
            return state
        return self.cycle_runner.run_next_step(state)

    def run_scheduled_cycle(self) -> CycleState:
        """Run a scheduled assistant cycle using configuration.

        Returns:
            Final CycleState.

        Raises:
            ValueError: If scheduled mode is disabled or if improvement mode
                has no golden projects configured (blocking requirement).
        """
        scheduled = self.runner_config.scheduled
        if not scheduled.enabled:
            raise ValueError("Scheduled assistant mode is not enabled")

        parsed_golden = parse_golden_projects_input(
            scheduled.golden_projects, scheduled.project_root
        )

        if scheduled.max_improvements_per_cycle > 0 and not parsed_golden:
            raise ValueError(
                "Scheduled cycles in IMPROVEMENT MODE require golden projects. "
                "Configure golden_projects or set max_improvements_per_cycle=0 for learning mode."
            )

        config = CycleConfig(
            project_root=scheduled.project_root,
            task_description="Scheduled maintenance cycle",
            max_improvements=scheduled.max_improvements_per_cycle,
            max_runtime_hours=scheduled.max_runtime_hours,
            stop_on_convergence=scheduled.stop_on_convergence,
            golden_projects=parsed_golden,
        )

        return self.cycle_runner.run_full_cycle(config, AssistantMode.SCHEDULED_ASSISTANT)

    def get_cycle_status(self, cycle_id: Optional[str] = None) -> Optional[dict[str, Any]]:
        """Get status of a cycle.

        Args:
            cycle_id: Specific cycle ID, or None for latest.

        Returns:
            Cycle status dictionary.
        """
        try:
            if cycle_id:
                state = self.cycle_store.load(cycle_id)
            else:
                state = self.cycle_store.get_latest()
            ...
            return {
                ...
                "mode": state.mode,
                "current_step": state.current_step().value if state.current_step() else "complete",
                "steps_completed": state.current_step_index,
                "total_steps": len(state.steps),
                "completed": state.completed,
                "interrupted": state.interrupted,
                "improvements_approved": state.improvements_approved,
                "improvements_rejected": state.improvements_rejected,
                "regressions_detected": state.regressions_detected,
                "convergence_reached": state.convergence_reached,
    ...
    def get_morning_summary(self) -> dict[str, Any]:
        """Get the morning summary of recent cycles.

        Returns:
            Summary dictionary.
        """
        return self.cycle_runner.get_morning_summary()

    def interrupt_current_cycle(self, reason: str = "User interrupted") -> Optional[CycleState]:
        """Interrupt the current running cycle.

        Args:
            reason: Reason for interruption.

        Returns:
            Updated CycleState or None.
        """
        state = self.cycle_store.get_latest()
        if state and not state.is_finished():
            return self.cycle_runner.interrupt_cycle(state, reason)
        return None
