...
def run_command(
    ...
    Args:
        command: The command to execute (string or list of args).
                 If string and shell=False, will be parsed with shlex.split().
        working_dir: Working directory for the command.
        timeout: Timeout in seconds.
        shell: Whether to run through shell. Default False for security.
               Only set True for commands requiring shell features (pipes, globs).
...
def run_pytest(
    ...
    return run_command(cmd_parts, working_dir=working_dir, timeout=300)
...
def run_ruff(
    ...
    return run_command(cmd_parts, working_dir=working_dir, timeout=60)
...
def run_diagnostics(
    ...
    if include_mypy:
        mypy_cmd = [sys.executable, "-m", "mypy", path, "--ignore-missing-imports"]
...
def run_typecheck(
    ...
    """
    cmd = [sys.executable, "-m", "mypy", path, "--ignore-missing-imports", "--no-error-summary"]
