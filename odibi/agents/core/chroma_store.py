...
class ChromaVectorStore(BaseVectorStore):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "chromadb is required for local vector storage. Install with: pip install chromadb"
            )
    ...
    def count(self, repo: str | None = None) -> int:
        ...
        try:
            if repo:
                result = self.collection.get(where={"repo": repo}, include=[])
    ...
    def delete_repo(self, repo: str) -> bool:
        ...
        Args:
            repo: Repository name to delete.
        ...
        try:
            ...
            if result["ids"]:
                ...
                logger.info(f"Deleted {len(result['ids'])} chunks from repo: {repo}")
            return True
        except Exception as e:
            logger.error(f"Failed to delete repo {repo}: {e}")
            return False
