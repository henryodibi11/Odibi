...
def _get_default_memories_path() -> str:
    """Get the default memories path from env or relative detection."""
    env_root = os.environ.get("ODIBI_ROOT")
    if env_root:
        return f"{env_root}/.odibi/memories"

    current = Path(__file__).resolve()
    for parent in current.parents:
        if (parent / ".odibi").is_dir():
            return str(parent / ".odibi" / "memories")
        if (parent / "agents").is_dir() and (parent / ".git").exists():
            return str(parent / ".odibi" / "memories")

    return "d:/odibi/.odibi/memories"
...
class MemoryScope(str, Enum):
    ...
    Validated: Durable, production memories from successful improvement cycles.
    Learning: Ephemeral, non-validated memories from observation-only cycles.

    Learning memories:
    - Are stored separately from validated memories
    - Are labeled as non-validated
    - May be discarded or promoted after review
    - Do not pollute the production curriculum
    """
...
class Memory:
    ...
    scope: MemoryScope = MemoryScope.VALIDATED
    ...
    def from_dict(cls, data: dict[str, Any]) -> "Memory":
        ...
        scope_str = data.get("scope", MemoryScope.VALIDATED.value)
        try:
            scope = MemoryScope(scope_str)
        except ValueError:
            scope = MemoryScope.VALIDATED
...
class MemoryStore:
    ...
    def __init__(
        ...
        **backend_kwargs,
        ...
        self._base_local_path = local_path or _get_default_memories_path()

        if backend_type == "local":
            self.backend = create_memory_backend(
                "local",
                base_path=self._base_local_path,
            )
            # Separate backend for learning scope memories
            self._learning_backend = create_memory_backend(
                "local",
                base_path=f"{self._base_local_path}/learning",
            )
        else:
            self.backend = create_memory_backend(
                backend_type,
            ...
            self._learning_backend = self.backend  # Use same backend for non-local
    ...
    def store(self, memory: Memory) -> bool:
        ...
        Note:
            Learning scope memories are stored separately from validated memories.
            This prevents observation-only cycles from polluting the production
            curriculum with non-validated patterns.
        """
        if memory.scope == MemoryScope.LEARNING:
            return self._learning_backend.save(memory.id, memory.to_dict())
