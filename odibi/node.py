...
class NodeResult(BaseModel):
    ...
    result_schema: Optional[List[str]] = Field(default=None, alias="schema")  # Renamed to avoid shadowing
...
class Node:
    ...
    def execute(self) -> NodeResult:
        ...
        try:
            ...
            if self.config.transform:
                ...
                self._execution_steps.append(f"Applied {len(self.config.transform.steps)} transform steps")
    ...
    def _generate_suggestions(self, error: Exception) -> List[str]:
        ...
        if "column" in error_str and "not found" in error_str:
            suggestions.append(
                f"Check that previous nodes output the expected columns"
            )
            suggestions.append(
