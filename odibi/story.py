...
class StoryGenerator:
    ...
    def generate(
        ...
        if failed:
            ...
            lines.append(f"**Status:** ⚠️ No nodes executed")
        ...
        for node_name in all_nodes:
            if node_name in node_results:
                ...
                node_section = self._generate_node_section(
                    node_name,
                    node_result,
                    context
                )
    ...
    def _generate_node_section(
        ...
        if result.result_schema:
            ...
            lines.append(f"- Columns ({len(result.result_schema)}): {', '.join(result.result_schema)}")
