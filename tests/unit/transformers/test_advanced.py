...
def nulls_df():
    ...
    return pd.DataFrame(
        {"id": [None, None, None], "name": [None, None, None], "value": [None, None, None]}
    )
...
def test_deduplicate_no_duplicates(sample_df):
    ...
    pd.testing.assert_frame_equal(
        result_ctx.df.sort_values("id").reset_index(drop=True),
...
def test_explode_list_column_empty_lists():
    ...
    df = pd.DataFrame({"id": [1, 2], "items": [[], ["a"]]})
    context = setup_context(df)
...
def test_dict_based_mapping_no_default():
    ...
    context = setup_context(df)
    params = DictMappingParams(column="code", mapping={"A": "Apple", "B": "Banana"})
    result_ctx = dict_based_mapping(context, params)
...
def test_dict_based_mapping_inplace():
    ...
    context = setup_context(df)
    params = DictMappingParams(column="status", mapping={"1": "Active", "0": "Inactive"})
    result_ctx = dict_based_mapping(context, params)
...
def test_dict_based_mapping_empty(empty_df):
    ...
    context = setup_context(df)
    params = DictMappingParams(column="status", mapping={"1": "Active"})
    result_ctx = dict_based_mapping(context, params)
    assert result_ctx.df.empty
...
def test_regex_replace_basic():
    ...
    df = pd.DataFrame({"phone": ["123-456-7890", "987-654-3210"], "id": [1, 2]})
    context = setup_context(df)
    params = RegexReplaceParams(column="phone", pattern="[^0-9]", replacement="")
...
def test_regex_replace_empty(empty_df):
    ...
    context = setup_context(df)
    params = RegexReplaceParams(column="text", pattern="[0-9]", replacement="X")
    result_ctx = regex_replace(context, params)
    assert result_ctx.df.empty
...
def test_regex_replace_nulls():
    ...
    df = pd.DataFrame({"text": ["abc123", None, "xyz456"]})
    context = setup_context(df)
    params = RegexReplaceParams(column="text", pattern="[0-9]", replacement="X")
    result_ctx = regex_replace(context, params)
...
def test_unpack_struct_basic():
    ...
    df = pd.DataFrame(
        {"id": [1, 2], "user_info": [{"name": "Alice", "age": 30}, {"name": "Bob", "age": 25}]}
    )
    context = setup_context(df)
...
def test_unpack_struct_nulls():
    ...
    df = pd.DataFrame({"id": [1, 2], "data": [{"key": "val"}, None]})
    context = setup_context(df)
    params = UnpackStructParams(column="data")
    result_ctx = unpack_struct(context, params)
...
def test_hash_columns_multiple():
    ...
    df = pd.DataFrame(
        {"email": ["a@b.com", "c@d.com"], "ssn": ["123-45-6789", "987-65-4321"], "id": [1, 2]}
    )
    context = setup_context(df)
...
def test_generate_surrogate_key_basic(sample_df):
    ...
    context = setup_context(sample_df)
    params = SurrogateKeyParams(columns=["id", "name"], separator="-", output_col="sk")
...
def test_generate_numeric_key_basic(sample_df):
    ...
    context = setup_context(sample_df)
    params = NumericKeyParams(columns=["id", "name"], output_col="numeric_key")
...
def test_generate_numeric_key_with_coalesce():
    ...
    context = setup_context(df)
    params = NumericKeyParams(columns=["name"], output_col="id", coalesce_with="id")
    result_ctx = generate_numeric_key(context, params)
...
def test_parse_json_basic():
    ...
    df = pd.DataFrame({"data": ['{"key": "value"}', '{"key": "other"}']})
    context = setup_context(df)
    params = ParseJsonParams(column="data", json_schema="key STRING", output_col="parsed")
...
def test_parse_json_empty(empty_df):
    ...
    context = setup_context(df)
    params = ParseJsonParams(column="json_col", json_schema="a INT")
    result_ctx = parse_json(context, params)
    assert result_ctx.df.empty
...
def test_validate_and_flag_basic():
    ...
    df = pd.DataFrame({"age": [25, -5, 30], "email": ["a@b.com", "invalid", "c@d.com"]})
    context = setup_context(df)
    params = ValidateAndFlagParams(
        rules={"age_check": "age >= 0", "email_format": "email LIKE '%@%'"}, flag_col="issues"
    )
...
def test_validate_and_flag_empty(empty_df):
    ...
    context = setup_context(df)
    params = ValidateAndFlagParams(rules={"check": "value > 0"})
    result_ctx = validate_and_flag(context, params)
    assert result_ctx.df.empty
...
def test_validate_and_flag_all_pass():
    ...
    context = setup_context(df)
    params = ValidateAndFlagParams(rules={"positive": "value > 0"})
    result_ctx = validate_and_flag(context, params)
...
def test_window_calculation_no_partition():
    ...
    df = pd.DataFrame({"value": [1, 2, 3]})
    context = setup_context(df)
    params = WindowCalculationParams(target_col="row_num", function="row_number()")
    result_ctx = window_calculation(context, params)
...
def test_window_calculation_empty(empty_df):
    ...
    df = pd.DataFrame(columns=["value"])
    context = setup_context(df)
    params = WindowCalculationParams(target_col="result", function="sum(value)")
    result_ctx = window_calculation(context, params)
    assert result_ctx.df.empty
...
def test_normalize_json_basic():
    ...
    df = pd.DataFrame(
        {"id": [1, 2], "data": [{"name": "Alice", "age": 30}, {"name": "Bob", "age": 25}]}
    )
    context = setup_context(df)
...
def test_sessionize_empty(empty_df):
    ...
    context = setup_context(df)
    params = SessionizeParams(user_col="user", timestamp_col="ts")
    result_ctx = sessionize(context, params)
    assert result_ctx.df.empty
...
def test_sessionize_single_user():
    ...
    context = setup_context(df)
    params = SessionizeParams(user_col="user", timestamp_col="timestamp")
    result_ctx = sessionize(context, params)
...
def test_split_events_by_period_day():
    ...
    df = pd.DataFrame(
        {"event_id": [1], "start": ["2023-01-01 23:00:00"], "end": ["2023-01-02 01:00:00"]}
    )
    context = setup_context(df)
...
def test_split_events_by_period_hour():
    ...
    df = pd.DataFrame(
        {"event_id": [1], "start": ["2023-01-01 10:30:00"], "end": ["2023-01-01 12:30:00"]}
    )
    context = setup_context(df)
...
def test_split_events_by_period_shift():
    ...
    df = pd.DataFrame(
        {"event_id": [1], "start": ["2023-01-01 07:00:00"], "end": ["2023-01-01 15:00:00"]}
    )
    context = setup_context(df)
...
def test_split_events_by_period_invalid_period():
    ...
    df = pd.DataFrame({"start": ["2023-01-01"], "end": ["2023-01-02"]})
    setup_context(df)

    # Pydantic validation will catch this before the function is called
    with pytest.raises(ValidationError):
        SplitEventsByPeriodParams(start_col="start", end_col="end", period="invalid")
