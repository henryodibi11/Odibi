...
class TestSCD2PatternExecution:
    ...
    def test_execute_calls_scd2_transformer(
        self, mock_scd2, mock_engine, mock_config
    ):
        ...
        source_df = pd.DataFrame(
            {"customer_id": [1, 2], "name": ["Alice", "Bob"], "updated_at": ["2024-01-01", "2024-01-02"]}
        )
    ...
    @patch("odibi.patterns.scd2.scd2")
    def test_execute_uses_default_column_names(
        self, mock_scd2, mock_engine, mock_config
    ):
    ...
    @patch("odibi.patterns.scd2.scd2")
    def test_execute_filters_valid_params(
        self, mock_scd2, mock_engine, mock_config
    ):
    ...
    @patch("odibi.patterns.scd2.scd2")
    def test_execute_handles_spark_context(
        self, mock_scd2, mock_engine, mock_config
    ):
    ...
    @patch("odibi.patterns.scd2.scd2")
    def test_execute_handles_count_failure(
        self, mock_scd2, mock_engine, mock_config
    ):
...
class TestSCD2PatternErrorHandling:
    ...
    def test_execute_raises_on_missing_required_params(
    ...
    @patch("odibi.patterns.scd2.scd2")
    def test_execute_raises_on_scd2_failure(
        self, mock_scd2, mock_engine, mock_config
    ):
...
class TestSCD2PatternLogging:
    ...
    @patch("odibi.patterns.scd2.scd2")
    @patch("odibi.patterns.scd2.get_logging_context")
    def test_execute_logs_row_counts(
        self, mock_get_ctx, mock_scd2, mock_engine, mock_config
    ):
...
class TestSCD2PatternConfiguration:
    ...
    def test_custom_column_names(self, mock_scd2, mock_engine, mock_config):
        ...
        source_df = pd.DataFrame(
            {"customer_id": [1, 2], "name": ["Alice", "Bob"], "effective_date": ["2024-01-01", "2024-01-02"]}
        )
        context = create_pandas_context(source_df, mock_engine)
