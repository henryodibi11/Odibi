...
class TestPipelineExecution:
    ...
    def test_simple_read_only_pipeline(self):
        ...
        connections = {
            "local": LocalConnection(base_path=str(self.test_path))
        }
            ...
            nodes=[
                ...
                NodeConfig(
                    ...
                    transform=TransformConfig(
                        steps=[
                            {"function": "double_values", "params": {"source": "load"}}
                        ]
                    )
                )
            ]
...
class TestPipelineFailures:
    ...
    def test_independent_nodes_continue_after_failure(self):
        ...
        pipeline_config = PipelineConfig(
            ...
            nodes=[
                NodeConfig(name="success1", read=ReadConfig(connection="local", format="csv", path="input.csv")),
                NodeConfig(
                    name="fail",
                    read=ReadConfig(connection="local", format="csv", path="input.csv"),
                    transform=TransformConfig(
                ...
                NodeConfig(name="success2", read=ReadConfig(connection="local", format="csv", path="input.csv"))
            ]
        )
...
class TestPipelineValidation:
    ...
    def test_validate_detects_missing_connections(self):
        ...
        pipeline_config = PipelineConfig(
            pipeline="test",
            nodes=[
                NodeConfig(name="node1", read=ReadConfig(connection="missing_conn", format="csv", path="a.csv"))
            ]
        )
