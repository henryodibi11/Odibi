    ...
    try:
        ...
        if "customer_id" in df.columns:
            ...
            if len(versioned) > 0:
                ...
                print(
                    f"  Example: customer_id {versioned.index[0]} has {versioned.iloc[0]} versions"
                )
            else:
                warnings.append(
                    "⚠️  No version history found (run pipeline twice with changed data)"
                )
        ...
        if "valid_from" in df.columns and "valid_to" in df.columns:
            ...
            if len(invalid_ranges) > 0:
                errors.append(
                    f"❌ {len(invalid_ranges)} rows have invalid date ranges (valid_from > valid_to)"
                )
            else:
