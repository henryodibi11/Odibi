...
def verify_fact_table():
    ...
    try:
        ...
        measures = [
            col for col in fact.columns if any(m in col.lower() for m in measure_candidates)
        ]
            ...
            for dim_path in dims:
                ...
                if sk_name in fact.columns:
                    try:
                        ...
                        if orphans_filtered:
                            errors.append(
                                f"❌ {len(orphans_filtered)} invalid {sk_name} values (not in {dim_name})"
                            )
                        else:
        ...
        degenerate_candidates = [
            ...
            if any(x in col.lower() for x in ["number", "id", "code"]) and not col.endswith("_sk")
        ]
