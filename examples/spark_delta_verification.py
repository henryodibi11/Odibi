...
def setup_environment():
    ...
    print(f"[INFO] Setting up Delta environment")
...
def run_delta_verification():
        ...
        "pipelines": [
            # 1. Initialize Table (Overwrite)
            {
                ...
                "nodes": [
                    {
                        "name": "write_v0",
                        "read": {"connection": "input_data", "path": "delta_v0.csv", "format": "csv", "options": {"header": True, "inferSchema": True}},
                        "write": {"connection": "output_data", "path": "main_table", "format": "delta", "mode": "overwrite"}
                    }
                ]
            },
            # 2. Upsert Data (Merge)
            {
                ...
                "nodes": [
                    {
                        "name": "write_v1",
                        "read": {"connection": "input_data", "path": "delta_v1.csv", "format": "csv", "options": {"header": True, "inferSchema": True}},
                        "write": {
    ...
    valid_upsert = (
        len(rows) == 2 and
        rows[0]["id"] == 1 and rows[0]["value"] == "B" and 
        rows[1]["id"] == 2 and rows[1]["value"] == "C"
    )
    ...
    valid_tt = (
        len(rows_v0) == 1 and
        rows_v0[0]["id"] == 1 and rows_v0[0]["value"] == "A"
    )
