...
- Fixed ReadConfig validator to recognize `sql_file` as valid data source
...
- **discover_database**: Now shallow by default
  - `max_tables`: 50 → 20
  - `sample_rows`: 5 → 0 (structure only, use preview_source for samples)

- **discover_storage**: Now shallow by default
  - `max_files`: 20 → 10
  - `sample_rows`: 5 → 0 (structure only, use preview_source for samples)
  - `recursive`: True → False (explicit opt-in for deep scanning)

- **Two-step discovery pattern** documented:
  1. `discover_*` returns structure only (lightweight)
  2. `preview_source` for detailed samples on specific items

This reduces token cost, prevents context explosion, and makes deep scanning intentional.
