...
You are working on **odibi**, a Python data pipeline framework for building enterprise data warehouses. It orchestrates nodes (read → transform → validate → write) with dependency resolution, supports Pandas/Spark/Polars engines, and provides 6 warehouse patterns, 57 transformers, and 11 validation test types.
