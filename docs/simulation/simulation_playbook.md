...
!!! info "Prerequisites"
    This playbook assumes you understand [Core Concepts](core_concepts.md) (scope, entities, columns) and have read the [Stateful Functions](stateful_functions.md) reference for `prev()`, `ema()`, and `pid()`. If not, start there — this page builds on them.
...
- **[Stateful Functions](stateful_functions.md)** — `prev()`, `ema()`, `pid()` complete reference
